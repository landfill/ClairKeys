...
- Decision:
  ...
  2. **`DEFAULT_MASTER_GAIN` 0.22 → 0.7.** 조밀한 화음이 −4.5 dBFS에 온다 — 정상 청취 레벨이면서 슬라이더가 더 갈 여지를 남긴다.
  3. **`MAX_MASTER_GAIN` 0.35 → 0.9.** 손가락은 열 개이므로 **동시타 12음이 이미 연주 가능 범위 밖**이고, 그보다 조밀한 텍스처는 페달로만 도달하는데 페달 누적은 피크가 더 낮다. 0.9에서 동시타 12음이 양쪽 경로 모두 1.0 바로 아래(0.974/0.977)다.
  4. **상한은 두 경로 중 큰 쪽(합성) 기준으로 잡는다.** 버스가 공유되므로 샘플 경로만 보고 정하면, 샘플에서는 안전해 보이는 설정에서 합성 패시지가 클리핑한다.
  5. **동시타 16음은 설계 목표가 아니라 바깥 경계로 기록한다.** 0.9에서 넘치지만, 일어나지 않는 텍스처에 상한을 맞추는 것이 바로 이 결함을 만든 실수다.
  6. **`SAMPLE_PEAK_GAIN`은 건드리지 않는다.** 두 경로의 상대 레벨은 D-015가 정했고 유효하다. 이 결정은 절대 단계만 움직이며 두 경로를 함께 올린다.
- Consequence (명시적으로 감수하는 것):
  - 기본 음량이 **+10.1 dB**, 슬라이더 상한이 **+8.2 dB** 오른다. 이미 재생 중인 사용자에게는 갑작스러운 변화다.
...
- Directive:
  ...
  - **믹스다운 실측값은 샘플 세트 `v1` 기준이다.** 세트를 재빌드하면 함께 다시 재라.
