...
## D-044: 방향성 진행에서는 손 끝 소모를 내부 이벤트 비용으로 반영해 D-041을 개정한다

- Date: 2026-09-05
- Status: Accepted
- Amends: D-041 (결정 1·2의 전이 비용 모델)
- Fulfills: 이슈 [#130](https://github.com/landfill/ClairKeys/issues/130) 3단계
- Context:
  - NATURAL_SPAN = [0,2,4,5,7]은 정확히 7반음이므로 왼손 E2:5 → B2:1의 implied anchor가 둘 다
    40이다. v3는 이 도약을 무료로 선택해 첫 음에서 방향 쪽 손가락을 모두 소모했고, 이후 2↔1 왕복을
    만들었다.
  - 손 위치를 DP 상태로 추가하면 단음 후보 5개가 25개 전이에서 100개 상태·10,000개 전이로 커진다.
    따라서 예산은 기존 후보와 앞뒤 이벤트에서 유도되는 비용이어야 하며, 원본 finger와 reach 제약은
    그대로 보존해야 한다.
- Decision:
  1. 손가락 후보가 방향 진행 쪽 끝(상행의 spatial 5, 하행의 spatial 1)에 도달하고 다음 이벤트도 같은
     방향으로 진행하는 내부 이벤트에는 DIRECTIONAL_BUDGET 비용을 부과한다. 마지막 이벤트에는 부과하지
     않아 정상적인 scale 종결 손가락을 이동으로 대체하지 않는다.
  2. 이 비용은 자연 손 폭 7반음 이내의 연속 진행에만 적용한다. 옥타브 이상 도약은 실제 손 재배치로
     취급하며 D-041의 같은 손가락 도약 예외를 유지한다.
  3. 후보 수·DP 차원·canonical v1.0/v1.1 계약을 바꾸지 않고, 유효한 원본 finger는 계속 덮어쓰지 않는다.
     화면 값이 바뀌므로 provenance 버전은 phrase-dp-v4로 올린다.
- Reason: 손 끝에 도달한 뒤 같은 방향으로 계속 연주할 수 있는 손가락 여유가 없다는 물리적 제약을 직접
  가격화해야, anchor 이동이 0인 완전5도가 장기 악구의 첫 선택으로 과대평가되지 않는다.
- Consequence:
  - 운영 corpus의 진행 중 손 재배치는 65/171 → 61/171로 감소했다. 도달 불가 화음 쌍은 0/154 유지,
    같은 손가락 3연속 run 수는 2 유지했다.
  - 3마디 왼손은 5 1 2 1 2 1 2에서 5 2 2 1 2 1 2가 되어 첫 완전5도에서 spatial 끝 손가락을
    쓰지 않는다. 이는 기존 결함 고정이 아니라, 원본 음을 보존하면서 손 끝 소모를 늦추는 동작 회귀다.
  - 자동값은 여전히 교육용 힌트이며 개인 손 크기나 전문 운지의 유일한 정답을 주장하지 않는다.
  - 2차 검증에서 최장 run과 음 변화 run을 새로 계측했다. v4의 4·2는 v3 기준 3·1을 후퇴한 값으로
    확인됐고, 이를 되돌리려는 지역 비용 실험은 재배치를 61/171에서 66/171로 올렸다. 따라서 이
    결정의 현재 비용은 추가 악구 정보 없이는 다섯 래칫을 동시에 개선하지 못하는 한계를 남긴다.
- Rejected:
  - 손 위치를 DP 상태 차원으로 추가 | 이슈 #126/D-041의 복잡도 제약을 위반한다.
  - 모든 방향 전이에 손가락 진행 비용을 더한다 | 표준 scale과 도약 예외를 훼손하고 corpus의 반복·재배치가
    악화되었다.
  - 옥타브 도약에도 방향성 예산을 적용한다 | 손을 실제로 옮기는 figure에서 D-041의 정당한 같은 손가락
    예외를 깨뜨린다.
- Constraint: D-038·D-039의 원본 운지 불변·결정론·저장 계약, D-043의 reach 하드 제약, 이슈 #130의 후보
  수와 성능 제약을 유지한다.
- Confidence: medium (손 끝 조기 소모의 기전과 corpus 개선은 높지만, 전문 연주자 전체 검수는 없음)
- Scope-risk: moderate
- Reversibility: clean
- Directive: 방향성 비용을 조정할 때는 3마디 아르페지오와 corpus 세 래칫을 함께 검증한다. 도달 한계는
  비용으로 되돌리지 말고, 옥타브 도약 예외를 방향성 예산의 대상으로 확장하지 않는다.
- Tested: 1차 구현은 focused Jest 3 suites / 66 tests, 전체 Jest 93 suites / 891 tests, tsc, lint, build,
  report와 12,000음 benchmark를 통과했다. 2차 계측은 v4의 최장 run 4·음 변화 run 2를 v3 기준 3·1과
  비교해 실패를 재현했고, 동일 손가락 도약 비용 실험은 재배치 61→66으로 악화됨을 확인했다.
- Not-tested: 전문 피아니스트 검수와 악구 경계·voice 정보를 포함한 후속 모델은 아직 검증하지 않았다.
- Related: D-038, D-039, D-041, D-043, 이슈 [#130](https://github.com/landfill/ClairKeys/issues/130)
