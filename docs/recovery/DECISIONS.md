...
## D-044: 운지 지표는 정석 운지로 검증한 뒤에만 최적화 목표가 된다 — `repositionsInMonotoneRuns`는 실격

- Date: 2026-09-05
- Status: Accepted
- Amends: D-042 (결정 3의 래칫에 검증 절차를 선행 조건으로 추가)
- Fulfills: 이슈 [#130](https://github.com/landfill/ClairKeys/issues/130) 3단계의 목표 재정의
- Context:
  - 3단계를 두 번 시도했고 둘 다 목표 지표를 거의 움직이지 못했다(65→61, 61→64). 기전이 틀린 게 아니라
    **목표가 틀렸다.**
  - 증거는 결정적이다. corpus 3마디 왼손(E2→F#4, 26반음 상행)에 여러 운지를 넣고 재보면:
    ...
    **모든 정석이 결함보다 나쁜 점수를 받는다.** 이 지표를 낮추는 최적화는 운지를 결함 쪽으로 민다.
  - 물리적 이유: 손 벌림보다 넓은 진행은 손이 반드시 이동해야 하고, 총 이동량은 음정 구간이 정한다
    (어느 운지든 21로 같았다). 지표는 그 총량이 몇 번에 나뉘는지만 셌다.
- Decision:
  1. **지표는 검증을 통과해야 목표가 될 수 있다.** `src/utils/__tests__/fingeringMetricValidity.test.ts`가
     소수의 참조 악구에 대해 "정석 운지가 결함 운지보다 나쁜 점수를 받지 않는다"를 검사한다. 참조 악구는
     **소리 내어 근거를 댈 수 있는 것만** 넣는다 — 커버리지가 아니라 방어 가능성이 기준이다.
  2. **`repositionsInMonotoneRuns`는 목표에서 실격**시키되 **삭제하지 않는다.** 역전 사실 자체를 통과하는
     테스트로 고정한다. 증거가 숫자보다 값지고, 나중에 이게 통과로 바뀌면 그건 성과가 아니라 뉴스다.
  3. **새 목표 지표 세 개**를 채택한다. 전부 검증을 통과했다.
     - `sameFingerLeaps` — 한 손가락이 미끄러질 수 없는 간격(4반음 초과)을 연속으로 맡는 경우
     - `wastedHandTravel` — 손이 **음정이 강제하는 것보다** 더 움직인 양. 생 이동량을 세면 악보 자신의
       도약(2옥타브 베이스 하강)이 잡히므로 하한(`|음정| - 손 너비`)을 빼고 잰다
     - `longestRepetitionRun` / `pitchChangingRepetitionRuns` — 반복 run의 개수가 아니라 길이와
       음 변화 여부. 개수만 세던 래칫이 v4의 3→4 후퇴를 통과시켰다
  4. **엄지 넘김은 낭비 이동에서 제외한다.** 넘김은 손이 건반에서 떨어지지 않고 이동하는 유일한 방법이므로
     작은 음정에 큰 anchor 이동이 따르는 것이 정상이다. 제외하지 않으면 정석 운지가 결함보다 나쁘게 나온다
     (실측으로 확인). 판정 함수 `isThumbCrossing`은 **해부학이므로 `src/utils/handReach.ts`에** 둔다 —
     계측이 비용 모델을 import하면 순환이 된다(D-043 결정 3).
- Consequence:
  - 새 기준선: 낭비 이동 **36/155**, 같은 손가락 도약 **10/155**, 최장 run 3, 음 변화 run 1, 도달 불가 0/154.
  - **두 이동 지표는 이어진 음에만 적용한다.** 둘 다 연결된 진행을 전제로 정의된다 — 한 손가락은 건반을
    누른 채 5반음을 미끄러질 수 없고, 음정이 요구하지 않은 손 이동은 레가토를 끊는다. 쉼표 뒤에는 둘 다
    성립하지 않는다. 이 조건이 없을 때 corpus는 **15.5초 떨어진 전이**를 결함으로 셌다(다른 악절이다).
    모델도 `splitPhrases`에서 같은 구분을 하고 있었으므로, 계측이 시간에 대해 모델과 다른 가정을 하고
    있던 셈이다. 임계값은 음악적 길이가 아니라 0보다 조금 큰 값이다 — 판단 기준은 쉼표의 길이가 아니라
    두 음이 붙어 있는가다. 반복 run 지표는 그대로 둔다: 같은 손가락을 반복해서 쓰는 사실은 쉼표가 있어도
    관찰 대상이다.
  - 새 지표가 사용자가 보고한 증상을 직접 가리킨다. `B2:1 → G#3:2`는 손이 11반음 움직이는데 음정은 2반음만
    요구한다 — "5도에 손을 다 쓰고 그 다음 도약한다"가 숫자로 잡힌다.
  - 넘김 제외는 277개 전이 중 19개만 면제하며, 그중 가장 넓은 것도 `E3:1 → A3:3`(음정 5, 이동 8)인 전형적
    tuck이다. 지표를 무력화하지 않는다.
  - 이슈 #130의 완료 조건에서 "재배치 감소"를 삭제하고 위 지표로 대체한다.
- Rejected:
  - 실격 지표를 삭제 | 역전 사실이 숫자보다 값지다. 다음 세션이 같은 벽에 부딪히는 것을 막는다.
  - 생 손 이동량을 목표로 | 악보 자신의 도약이 잡힌다. 25반음 베이스 하강은 어떤 운지든 손이 20반음 움직인다.
  - 넘김을 제외하지 않음 | 정석 운지가 결함보다 나쁘게 나온다. 실측으로 확인했다.
  - 지표 여러 개를 하나로 합산 | 한쪽 개선이 다른 쪽 악화를 가린다(D-042 Rejected).
- Constraint: D-042(래칫), D-043(계측은 비용 모델을 import하지 않는다), 이슈 #130 복잡도 제약
- Confidence: high
- Scope-risk: narrow
- Reversibility: clean
- Directive: **검증 세트에 넣어 통과시키기 전에는 어떤 운지 지표도 최적화 목표로 삼지 않는다.** 지표가
  움직이지 않을 때 기전을 의심하기 전에 목표부터 의심하라 — 이번에 두 번의 시도가 그렇게 소모됐다.
- Tested: 전체 Jest 896건(신규 6건 검증 하네스), `tsc --noEmit`, `next lint`, `next build`. 3마디 역전과
  넘김 제외 범위(19/277)를 직접 측정.
- Not-tested: 참조 악구가 4개뿐이다. 대위·옥타브 연속·빠른 패시지는 아직 세트에 없다. 새 지표가 그 텍스처에서도
  옳게 순위를 매기는지는 미확인.
...
- Date: 2026-09-05
- Status: Accepted
- Amends: D-044 (검증 절차에 "구별 능력"과 "결함이 사는 악구 포함" 요건 추가, `wastedHandTravel` 실격)
- Context:
  - D-044로 목표를 교체한 뒤 4단계(모델 변경)를 시도했고, 워커가 lookahead 가설을 여러 형태로 구현했으나
    `wastedHandTravel` 36/155를 하나도 낮추지 못했다(37~51, 51, 53으로 전부 악화). 실패로 처리하기 전에
    지표에 바닥이 있는지 직접 측정했다.
  - **또 뒤집혀 있었다.** corpus 3마디 왼손에서 모델의 결함 운지 `5 1 2 1 2`가 **2/4**, 정석
    `5 4 2 1 2`와 `5 3 2 1 3`이 각각 **3/4**다. 5^5 전수 탐색 1등은 **1/4**이고 그 운지는 `1 4 1 4 1` —
    26반음 아르페지오를 엄지와 약지로 교대하는, 아무도 치지 않는 답이다.
  - **D-044의 검증 하네스에 두 개의 구멍이 있었다.**
    1. **구별 능력을 검사하지 않았다.** 음계 악구에서 정석과 결함이 둘 다 0/7을 받는데,
       `worstGood > bestBad`만 보므로 `0 > 0`이 거짓이라 통과했다. 구별하지 못하는 지표가 통과한 것이다.
    2. **검증 세트에 아르페지오가 없었다.** 음계 3개와 도약 1개로만 구성했다. 이슈 #130이 다루는 바로 그
       악구를 넣자마자 `wastedHandTravel`이 즉시 실격했다.
  - **손 이동을 재는 지표 세 개가 독립적으로 같은 결론을 낸다** — `repositionsInMonotoneRuns`,
    `wastedHandTravel`, 그리고 4단계에서 시도한 변형들이 모두 모델의 아르페지오 운지를 정석보다 좋게 매긴다.
    세 번 같은 방향이면 지표 각각의 결함으로 설명하기 어렵다.
- Decision:
  1. **`wastedHandTravel`을 목표에서 실격**시킨다. `repositionsInMonotoneRuns`와 같이 삭제하지 않고 역전
     사실을 통과하는 테스트로 고정한다.
  2. **검증 하네스에 구별 능력 요건을 추가한다.** 목표로 admit되는 지표는 최소 한 악구에서 정석 < 결함으로
     **엄격히** 분리해야 한다. 역전하지 않는 것만으로는 부족하다.
  3. **검증 세트는 결함이 실제로 사는 악구를 포함해야 한다.** 결함이 없는 악구에서만 검증하면 그 지표가
     결함을 어떻게 다루는지 알 수 없다. 3마디 아르페지오를 세트에 추가했다.
  4. **남은 유효 목표는 셋이다** — `sameFingerLeaps`(10/155, 구별 1개 악구로 약함),
     `longestRepetitionRun`(3), `pitchChangingRepetitionRuns`(1).
  5. **4단계(아르페지오 운지 개선)를 미해결로 남긴다.** 무엇이 왜 잘못됐는지 정의할 근거가 없다.
     자세한 이유는 아래.
- Consequence:
  - **corpus 3마디의 `5 1 2 1 2`가 실제로 나쁜지 자체가 불확실하다.** 페달 밟은 26반음 왼손 아르페지오에서
    손가락 몇 개만 쓰고 손을 옮기는 것은 실제 왼손 주법이기도 하다. 세 지표가 모두 그쪽을 지지한다.
  - 측정으로 확실히 나쁘다고 말할 수 있었던 것은 이미 처리됐다: 도달 불가 화음 26→0(D-043).
    나머지 유효 지표는 이미 양호하다(반복 run 3/1, 같은 손가락 도약 10/155).
  - **다음 진전에는 사람의 기준점이 필요하다.** 실제 화면에서 이상해 보이는 마디와, 그 구간의 정답 운지가
    있어야 검증 세트에 넣을 항목이 생긴다. 사용자가 마디를 특정하는 것이 까다롭다고 확인했으므로, 이
    항목은 후속 확인 대상으로 남긴다(2026-09-05).
- Rejected:
  - 상수를 조정해 `wastedHandTravel`을 낮춘다 | 그 지표를 낮추는 최적화는 아르페지오에서 운지를 결함 쪽으로
    민다. 전수 탐색 1등이 `1 4 1 4 1`인 것이 그 증거다.
  - 실격 지표를 삭제한다 | 역전 사실이 숫자보다 값지다. D-044 결정 2와 동일한 이유.
  - 정답 운지를 코디네이터가 더 추측해서 검증 세트를 늘린다 | 이미 두 단계를 그렇게 소모했다. 지표와
    가정한 정석이 계속 어긋나는 상황에서 추측을 더하는 것은 같은 실패를 반복한다.
  - 아르페지오 개선을 강행한다 | 목표가 없으면 개선을 정의할 수 없고, 진전 여부를 측정할 수도 없다.
- Constraint: D-042(래칫), D-043(계측 독립), D-044(검증 선행)
- Confidence: high
- Scope-risk: narrow
- Reversibility: clean
- Directive: **검증 세트를 만들 때 결함이 실제로 관찰된 악구를 반드시 포함하라.** 그리고 지표는 역전하지
  않는 것만으로 admit하지 말고 **구별하는지**까지 확인하라. 아르페지오 운지를 다시 다루려면 먼저 사람이
  판단한 기준 운지를 확보하라 — 코디네이터의 추측을 근거로 삼지 마라.
- Tested: 전체 Jest 904건, `tsc --noEmit`, `next lint`, `next build`. 3마디 역전과 5^5 전수 탐색,
...
- Context:
  - 이슈 #146 stage3의 남은 작업은 업로드 폼 그룹핑이고, 그 완료 조건은 전부 기하 측정이다 —
    곡명·저작자가 실제로 한 줄인지, 드롭존이 실제로 줄었는지. jsdom은 레이아웃을 계산하지
    않으므로 jest로는 증명할 수 없다. 쪼개는 경계를 어디에 둘지는 이 결정의 범위가 아니고
    D-059가 정한다.
...
## D-059: 곡명·저작자를 쪼개는 경계는 기기 종류가 아니라 칸 폭이 정한다

- Date: 2026-09-12
- Status: Accepted
- Applies to: `src/components/upload/OMRUploadForm.tsx`의 필수 정보 행, `e2e/upload-form-grouping.spec.ts`
- Replaces: `docs/recovery/HANDOFF.md`가 stage3 지시에 쓴 "two-column on desktop and one column on
  mobile" 표현. 그 표현이 가리키던 의도(넓으면 두 칸, 좁으면 한 칸)는 유지하고, 판정 기준만
  기기 종류에서 폭으로 바꾼다.
- Context:
  - CSS에는 "데스크톱"과 "모바일"이 없다. 미디어 쿼리가 아는 것은 폭뿐이므로, 기기 용어로 적힌
    기준은 구현할 때 반드시 어떤 폭으로 번역된다. 그 번역을 기록하지 않으면 다음 세션이 같은
    문장을 다른 폭으로 번역한다.
  - PR157 리뷰가 844x390 가로 모드에서 두 칸이 되는 것을 "모바일인데 두 칸"이라고 지적했다.
    그 지적을 계기로 경계를 실측했고, 옆에서 실제 결함이 나왔다.
  - 실측 (Chromium, 이 저장소의 업로드 화면): 390px 한 칸 308px / 639px 한 칸 557px /
    640px 두 칸 **263px** / 768px 두 칸 327px / 844px 두 칸 365px.
  - 즉 경계를 `sm`(640px)에 두면 정확히 640px에서 각 칸이 263px가 되어, 390px 휴대폰이 한 칸으로
    받는 308px보다 **좁다**. 화면을 넓혔더니 입력칸이 좁아지므로 쪼개는 목적 자체가 사라진다.
- Decision:
  1. 판정 기준은 폭이다. 규칙은 **"쪼갠 칸이 휴대폰이 한 칸으로 받는 폭보다 좁아지지 않는다"**이고,
     기준선은 390px에서의 단일 칸 폭이다.
  2. 이 규칙을 만족하는 가장 낮은 Tailwind 브레이크포인트는 `md`(768px)다. 필수 정보 행은
     `grid gap-4 md:grid-cols-2`를 쓴다.
  3. 844x390 가로 모드는 두 칸을 유지한다. 그 폭에서 각 칸은 365px로 기준선보다 넓고, 세로가
     390px뿐인 화면에서 굳이 쌓으면 가장 부족한 자원을 더 쓴다.
  4. E2E가 경계를 양쪽에서 고정한다. 390 / 767은 한 줄로 쌓이고, 768은 두 칸이며 그때 칸 폭이
     기준선보다 넓다. 더불어 모든 뷰포트 케이스에서 "두 칸이면 각 칸이 기준선보다 넓다"를 단언한다.
- Reason: 기기 용어는 검증할 수 없다. "모바일에서 한 칸"은 어느 폭에서 판정하는지 말하지 않으므로
  테스트로 옮길 수 없고, 실제로 이 저장소에서 그 표현은 640px이라는 잘못된 경계로 번역됐다. 폭과
  칸 폭은 둘 다 측정되므로 규칙과 검증이 같은 언어로 쓰인다.
- Rejected:
  - 844x390을 한 칸으로 쌓는다 (리뷰 처방) | 칸이 이미 기준선보다 넓어 읽기 문제가 없고, 세로가 가장 부족한 화면에서 행을 늘린다. 기기 종류를 기준으로 삼으면 이런 역전을 막을 방법이 없다.
  - `sm`을 유지하고 640px 구간만 예외 처리 | 경계가 잘못된 위치라는 사실은 그대로 남고 규칙이 두 개가 된다.
  - 기준선을 고정 px 상수로 박는다 | 폼 폭이 바뀌면 기준선도 바뀐다. E2E가 390px에서 실측한 값을 기준선으로 쓴다.
- Constraint: 브레이크포인트는 Tailwind가 제공하는 값만 쓴다. 임의 폭을 쓰면 다른 화면과 경계가 어긋난다.
- Confidence: high
- Scope-risk: narrow
- Reversibility: clean
- Directive: 이 행의 브레이크포인트를 앞당기려면 먼저 그 폭에서 칸 폭을 재라. 기준선보다 좁아지면 E2E가 막는다. 다른 화면에 같은 2열 배치를 도입할 때도 기기 용어가 아니라 이 규칙을 인용한다.
...
## D-063: 오선에 흡수된 평평한 타이 끝은 표준 연결이 비었을 때만 넓혀 다시 찾고, 타이일 때만 받는다

- Date: 2026-09-15
- Status: Proposed; accepted when the #134 tie head-link PR merges. Not deployed
- Context:
  - [타이 곡선 조사](validation/2026-09-14-issue-134-tie-curves.md)의 기전 B: 보표 안의 평평한 렌즈형 타이는 오선이 남은 골격에서
    끝이 오선에 흡수된다. 곡선은 weed·등급을 통과하지만 끝이 머리에서 2.2 IL 이상 떨어져, `SlurLinker`의 수평 탐색
    범위 `coverageHExt` 2.0 IL 밖이 된다. 한쪽만 연결되면 orphan 검사에서 떨어져 clump가 아무것도 고르지 못한다.
  - 흡수된 끝은 가장 가까운 오선 중심에서 0.0–4.1px(interline 20px, 최대 0.21 IL) 떨어져 있다. 선 두께가 3–4px이라
    `SlursBuilder`의 `maxStaffLineDy` 0.2 IL은 선 가장자리에 걸친 끝(m4 위 타이 4.1px)을 놓친다.
  - 사용자는 2026-09-15 후보 1(머리 연결 단계 확장)을 골랐다. 후보 2(purge 예외, 기전 A)와 3(NO_STAFF 골격)은 범위 밖이다.
- Decision:
  1. `ClumpPruner.prune`에서 기존 `selectSlur`가 결과를 낸 clump는 그대로 둔다. 결과가 null일 때만 같은 system에서
     `selectStaffLineTie`로 다시 찾는다. 따라서 지금 슬러·타이가 선택되는 clump의 결과는 바뀌지 않는다.
  2. 재탐색은 수평 슬러의 끝 가운데 가장 가까운 오선(탭 보표 제외)에서 0.25 IL 이내인 끝에만 수평 확장
...
- Decision (검증할 후보):
  1. SymbolsLinker 시작에서 표준 크기·단일 보표·빔 있는 복수 머리 화음만 검사한다. 연결된 빔 그룹의 모든 화음도 같은 보표에 있어야 한다.
     그룹이 없거나 다른 보표가 포함되면 분리하지 않는다. mirror 머리는 제외한다.
...
- Context: Clair 남은 타이12개 중 m9 E3·G3와 m10 D5는 곡선이 올바른 두 머리에 연결됐는데도 `tie=false`다.
  ...
  사이 화음으로 취급된다(침범0.39/0.43). [재분류](validation/2026-09-19-issue-134-residual-ties.md).
- Decision (검증할 후보):
  1. `isSpaceClear`의 장애물 목록에서, 끝 화음과 같은 보표에 있고 두 화음 모두 기둥이 있으며 기둥 방향이 반대이고,
     머리 하나가 끝 머리와 가로로 두 머리 폭 중 작은 값의0.5 이상 겹치는 HeadChord만 제외한다.
  2. 같은 x는 그 끝과 같은 시점이므로 두 머리 "사이"가 아니다. 가로로 벗어난 화음·같은 방향 기둥·기둥 없는 화음·다른 보표 화음은
     기존 검사에 남긴다. 빔 그룹 금지와 침범 비율 한도0.25는 바꾸지 않는다.
  3. CURVES의 ClumpPruner와 LINKS의 checkStaffTie가 같은 함수를 쓰므로 두 단계에 함께 적용된다(D-068 결정5와 같은 위치).
  4. 곡선 검출·머리 연결·시스템 경계 짝짓기와 exported XML/JSON은 바꾸지 않는다.
- Rejected: 침범 한도0.25 상향 | 진짜 사이 화음이 있는 곡선까지 타이로 바꿀 수 있다
- Rejected: LINKS 판정만 변경 | 같은 규칙이 CURVES와 달라지면 D-068 예외와도 어긋난다. 영향은 corpus로 확인한다
- Confidence: medium
- Scope-risk: moderate
- Reversibility: clean
...
## D-072: 오선 purge가 한쪽 후보를 모두 지울 때는 붙임줄로만 받을 수 있게 남긴다

- Date: 2026-09-19
- Status: Proposed; 로컬 필수 검증 완료, PR 심사 대상. 병합·배포 미승인
- Context: Clair 남은 타이9개 중 m3 G4·m13 G3·m14 D3는 끝이 머리가 놓인 오선에서 약0.2 IL 떨어져 평평하게 끝난다.
  `SlursBuilder.purgeStaffLines`는 끝이 오선에서 0.2 IL 이내이고 마지막0.5 IL의 입사각이15° 이하이면 "오선 조각"으로 지운다.
  곡선은 씨앗에서 오른쪽·왼쪽으로 늘린 두 후보 목록이 모두 남아야 만들어지는데, 이 세 타이는 한쪽(m14는 양쪽) 목록이 비어 곡선이 없다.
  운영 d071 진단 빌드에서 #19 (1630,898)–(2071,897), #51 (358,3081)–(691,3082), #96–#100 (1897,2500)–… 삭제를 재현했다.
  [재분류](validation/2026-09-19-issue-134-residual-ties.md).
- Decision (검증할 후보):
  1. 입사각 규칙으로 지워질 후보가 그 방향 목록을 전부 비울 때만 그 후보들을 남긴다. 다른 후보가 남는 목록과 끝 조각이
     STAFF_ARC인 후보는 기존대로 지운다. 남긴 후보의 끝점은 방향별로 기억한다(곡선마다 초기화).
  2. 남긴 끝을 포함한 clump는 ClumpPruner의 표준 선택을 건너뛰고 D-063 오선 타이 경로(`selectStaffLineTie`)만 쓴다.
     그 경로는 양쪽 같은 음높이·같은 보표 머리와 사이 공간을 요구하고 붙임줄이 아니면 거부한다. 이음줄로는 받지 않는다.
  3. 연결할 머리에 이미 같은 쪽(시작/끝) 타이가 있으면 받지 않는다. 이 조건이 없으면 Clair m5→m6 G4의 기존 타이 옆 짧은 조각
     (1689,1211)–(1725,1218)이 두 번째 타이가 되어 MusicXML에 tie start/stop이 중복됐다.
  4. 남긴 곡선은 현에서 곡선까지의 최대 거리(휜 높이)가 0.15 IL 이상이어야 한다. 1차 corpus에서 이 조건 없이 Always With Me m8·m21과
     Merry Go Round m31의 곧은 오선 구간(휜 높이0.07–0.10 IL)이 같은 음높이 두 머리 사이 타이가 됐다. 진짜 타이는 0.22–0.51 IL이었다.
  5. 곡선 검출 이후의 머리 연결·타이 판정 규칙과 exported XML/JSON은 바꾸지 않는다.
- Rejected: 입사각/거리 한도 완화 | 오선을 따라 늘어난 가짜 후보가 짧은 정상 후보를 이겨 이음줄 연결이 바뀔 수 있다
- Rejected: 곡선 전체 휨만으로 예외 | 머리 근거 없이 오선 조각 이음줄을 새로 만들 수 있다
- Confidence: medium
- Scope-risk: moderate
- Reversibility: clean
- Rejected: 휜 높이 조건 없는 1차 후보 | corpus2곡에서 오선 구간 가짜 타이3개
- Tested: 기준선 d071 normal/recovery에서 새 native fixture가 m1 실패(보표 안 타이4개 누락). 후보 양쪽 통과
- Tested: 최종 d072b 정식 빌드, 두 엔진 jar2433항목 중 곡선 클래스6개만 변경·엔진 간 동일. 이미지191개 중185통과·6진단skip, native14 실행
- Tested: Clair3회191/191·타이34→37/43·오검출0·raw 해시 동일. 기존 곡선52개 불변, 목표 곡선3개만 추가
- Tested: corpus12곡 새 쌍. 10곡 동일, 드뷔시 달빛 m21→m22 B2 타이1개 추가(인쇄된 위쪽 호, 위 머리끼리 연결 확인), truongca 같은 SCALE 실패
- Not-tested: 운영 배포, corpus 밖 판형. 오선 구간 가짜 타이의 합성 재현은 8가지 배치에서 실패해 실제 corpus로만 검증했다
- Known limits: 곡선 가운데가 오선에 닿아 쪼개지는 경우(m12)와 시스템 경계 반쪽은 처리하지 않는다.
  남긴 곡선을 붙임줄로만 받으므로 같은 모양의 이음줄 누락은 그대로다. 중복 방지는 기존 타이가 먼저 확정되는 씨앗 순서에 기댄다.
- Directive: purge 예외를 이음줄로 넓히지 않는다. 출력 사후 보정은 하지 않는다
- Related: #134, D-063, D-071, validation/2026-09-19-issue-134-residual-ties.md
...
- Context: d072b 기준선은191/191·타이37/43이다. 로그 전용 진단은 raw 결과가 기준선과 같으며,
  ...
  D-063/D-072의 양끝 머리 조건을 통과하지 못한다. m10 E5의 작은 호는 모델 확장·최소 폭에서 탈락한다.
- Decision (검증할 후보):
  1. 기존 이음줄 짝짓기 전에 같은 보표 인덱스·같은 머리 음높이·같은 휨 방향인 타이 후보 쌍을 검증한다.
     기존 경계 위치·곡선 호환성 조건과 실제 `checkCrossTie` 의미를 유지하고 애매한 경쟁 후보는 임의 확정하지 않는다.
  2. 오선에서 잘린 머리 연결 또는 짧은 호의 제한적 복원이 필요하면 시스템 시작/끝 영역에 국한한다.
     원본 잉크에서 이미 얻은 곡선만 사용하고 상대 반쪽 및 두 머리 근거가 없는 복원 후보는 버린다.
  3. 실제 구현은 Part의 기존 탐욕 매칭 전에 `canExtend`·휨 방향·`areTieCompatible`(음이름/옥타브/임시표)를
     만족하는 서로 유일한 쌍을 우선 예약한다. 나머지 기존 후보는 원래 매칭을 유지한다.
     D-063의 확장 영역에서 정확히 한 머리만 연결된 경계 곡선은 `boundary-tie-only`로 표시한다.
     이 표시는 OMR에 보존하며 일반 이음줄 매칭을 금지하고, 최종 음높이 판정이 달라지면 두 반쪽을 제거한다.
  4. 최소 폭 예외는 staff header 끝부터4 IL 이내, 보표 위/아래1 IL 이내의 시작 반쪽에만 적용한다.
     원래0.7 IL 미만 후보 중0.5 IL 초과 폭만 그 범위에 맞춘 폭 점수로 평가한다. 나머지 원형 모델·높이·
     purge·머리 연결 조건은 유지한다. 이 후보도 경계 타이 전용 표시를 붙여 상대 없는 곡선을 허용하지 않는다.
  5. 보통 오선 조각을 이음줄로 허용하지 않는다. D-072의 가짜 타이 방지·동일 쪽 중복 방지를 유지하고,
     normal/recovery 양쪽에 같은 코드를 적용한다. XML/JSON 사후 보정은 금지한다.
  6. 새 복원 반쪽은 페이지 내부 시스템 경계에만 허용한다. 첫 시스템의 왼쪽과 마지막 시스템의 오른쪽은 제외한다.
     연결 후 기존 `isBeginningOrphan`/`isEndingOrphan` 분류도 요구한다. `canBeOrphan`의 경계 거리 검사만으로는
     좁은 마디의 절반 조건이나 페이지 바깥 끝이 이후 orphan 매칭/삭제를 거친다고 보장할 수 없기 때문이다.
     기존 검출 반쪽의 페이지 간 연결은 유지하지만 새 복원 반쪽의 페이지 간 확장은 이번 범위 밖이다.
  7. 각 기전별 실패 fixture·정상 및 부정 대조군, Clair 반복·corpus 모든 차이의 원본 대조로 채택 여부를 판단한다.
- Rejected: 출력 tie 플래그 보충 또는 특정 마디/좌표 분기 | 원본 인식 실패를 숨기고 일반화되지 않는다
- Rejected: 전역 최소 곡선 폭·반지름·머리 거리 한도 완화 | 정상 기호·오선 조각의 오검출 위험이 있다
- Confidence: medium
- Scope-risk: moderate
- Reversibility: clean
