...
- Decision:
  ...
  6. **2026-09-07 D-055로 표시 기하만 대체됨.** 기존 결정: 화면은 재생 전과 재생 중 모든 유효한 재생 음에 번호를 렌더한다. 번호 높이보다 짧은 음표는 번호를
...
## D-055: 운지 배지는 노트 안에 담고 최소 가독 크기 미만이면 표시만 생략한다

- Date: 2026-09-07
- Status: Accepted
- Context: #124에서 14px 정사각 배지가 10.49px 검은건반 노트를 넘고, 짧은 노트 위로
  돌출한다. D-038의 무조건 표시 규칙은 이웃 노트를 가리는 결과를 만든다.
- Decision:
  1. D-038 결정 6과 ISSUE-103의 무조건 짧은 노트 표시 요건을 대체한다. 운지 데이터는
     보존하고 표시만 제한한다. 손·음표·타이밍·기존 건반 기하는 변경하지 않는다.
  2. 배지 폭은 min(20px, 실제 노트 폭), 높이는 min(20px, max(14px, 노트 폭 × 0.7),
     실제 노트 높이)다. 가로 중앙, 아래쪽 여백은 최대 4px이되 남는 높이의 절반 이하로 한다.
     사각형이 노트 밖으로 나가지 않는다. 흰/검은 lane의 차이는 기존 실제 노트 폭에 이미 반영돼
     있으므로 별도 건반 종류 입력이나 폭 변경을 추가하지 않는다.
  3. 표시 하한은 노트 폭 10px, 높이 14px이다. 글자는 12~16px, line-height 1,
     고정 monospace 굵은 숫자를 사용한다. 좁은 10px 배지에서도 12px 숫자 한 자를 담는다.
     어두운 배경판으로 대비를 확보하고 외부로 번지는 text shadow는 제거한다.
  4. 140px/s의 0.1초 노트는 14px 높이이므로 폭이 충분하면 번호를 표시한다.
     더 짧거나 좁은 경우 숫자를 작게 줄이거나 이웃 노트에 겹치지 않고 생략한다.
     이는 제품 표시 기준이며 휴먼 실측으로 보편적 가독성을 입증했다는 주장은 아니다.
- Rejected: 14px 원형 배지를 노트 밖에 유지 | 좁은 lane에서 이웃 숫자와 충돌한다.
- Rejected: 노트/건반 폭을 확대 | #125 기하 범위와 재생 표시 계약을 불필요하게 변경한다.
- Validation: 폭 10.8/24/24.46/36px의 실제 건반 layout, 0.1초와 더 짧은 노트,
  컴포넌트 표시/생략 및 브라우저 데스크톱/모바일 가로 검증을 수행한다.
- Related: #124, D-038, D-037, ISSUE-124-finger-badge
