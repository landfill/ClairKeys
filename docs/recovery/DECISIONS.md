...
- Date: 2026-09-20
- Status: Proposed; 원인 추적 및 회귀 fixture 작성 중, 구현·최종 검증 미완료
...
- Decision (검증할 후보):
  ...
  3. 보통 오선 조각을 이음줄로 허용하지 않는다. D-072의 가짜 타이 방지·동일 쪽 중복 방지를 유지하고,
     normal/recovery 양쪽에 같은 코드를 적용한다. XML/JSON 사후 보정은 금지한다.
  4. 각 기전별 실패 fixture·정상 및 부정 대조군, Clair 반복·corpus 모든 차이의 원본 대조로 채택 여부를 판단한다.
...
- Not-tested: 구현·회귀 fixture 판별·전체 Docker/corpus/CI/리뷰
