...
| `src/components/library/`에 `alert()` 없음 | `grep` 0건 (`role="alert"` 2곳은 오류 전달용으로 유지) |
  ...
  — 2026-08-30 D-030 주: 위 문단은 DS-4 완료 시점(개정 전)의 기록이다. 그때의 "접근성·반응형
  검증"은 지금 "반응형 검증"이고, "완료 조건 7·8"은 개정 전 번호다. WCAG였던 구 조건 7은
  삭제됐고 구 조건 8(종단 완주)이 현재 조건 7이며, 그 종단 판정만 DS-7 소유로 남는다.
