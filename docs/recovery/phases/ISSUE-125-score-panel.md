...
3. 앱이 XML+매핑을 비공개 보관·재생 권한과 같은 조회(공개 악보는 로그인 무관, D-075)·악보 삭제와 함께 제거. 원본 PDF는 기존 임시 정리 유지.
...
- 신규 MusicXML 비공개 저장·권한·조회·삭제·오류 및 기존 악보 호환 테스트 통과.
  공개 악보는 비로그인·비소유자·소유자 모두 악보 패널을 쓰고, 비공개 악보는 소유자 외 404(D-075). 완성 JSON UTF-8은4MiB 이하로 제한해 호스팅 응답한도 안에 둔다.
