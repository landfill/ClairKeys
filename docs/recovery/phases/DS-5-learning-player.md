...
- 1차 컨트롤 전부 키보드로 조작 가능하고 포커스 순서가 시각 순서와 일치한다.
- 현재 음·다음 음·양손 구분이 **색상 외 형태**(모양·테두리·패턴)로도 구분된다 — WCAG 1.4.1.
- 재생 상태 변화가 `aria-live`로 전달된다.
- `prefers-reduced-motion`을 존중한다 (낙하 애니메이션 자체는 기능이므로 제거하지 않되,
  부수 모션은 줄인다).
...
- 양손·현재 음·다음 음이 색상 외 수단으로 구분된다 (회귀 테스트로 고정).
