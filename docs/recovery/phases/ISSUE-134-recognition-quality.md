...
- 2026-09-19: User explicitly approved PR171 merge, branch cleanup and VM deployment. Rechecked head `3143f2d` CI/review/CLEAN and
...
### 같은 시점 다른 성부가 막는 타이3 (D-071)

- 2026-09-19 재분류의 기전 E만 대상이다: 곡선과 양끝 머리 연결은 정상인데 `isSpaceClear`가 끝 머리와 같은 x의
  반대 기둥 방향 화음을 사이 화음으로 보고 타이를 거부한다(m9 E3·G3, m10 D5).
- 기준선에서 실패하는 생성 악보 native fixture(왼쪽 끝·오른쪽 끝 침범과 진짜 사이 화음 대조군)부터 작성한다.
- 판정 규칙만 바꾸고 곡선 검출·머리 연결·짝짓기와 출력 XML/JSON은 바꾸지 않는다.
- normal/recovery 전체 빌드·native 전체·Clair3회(목표 타이34/43, 이벤트191/191 유지)·타이 개별 목록·12곡 corpus·
  최신 head CI·리뷰까지 검증한다. 기전 A·m12·시스템 경계는 제외하며, 병합/VM배포는 별도 승인이다.

### 오선에 붙은 타이 끝 purge 3 (D-072)

- 2026-09-19 재분류의 기전 A만 대상이다: m3 G4·m13 G3·m14 D3 타이는 끝이 머리의 오선에서 약 0.2 IL 떨어져 평평하게 끝나
  `SlursBuilder.purgeStaffLines`가 "오선 조각"으로 보고 그 방향 후보를 전부 지워 곡선 자체가 생기지 않는다.
- 기준선에서 실패하는 생성 악보 native fixture(오선에 붙은 위 타이 4, 보표 밖 아래 타이 대조군 4)부터 작성한다.
- purge가 한쪽 후보를 전부 비울 때만 입사각으로 지워질 후보를 남기고, 그 곡선은 D-063 오선 타이 경로로 붙임줄일 때만 받는다.
  이미 같은 쪽 타이가 있는 머리에는 받지 않는다. 곡선이 오선과 교차해 쪼개지는 m12와 시스템 경계는 제외한다.
- normal/recovery 전체 빌드·native 전체·Clair3회(목표 타이37/43, 이벤트191/191 유지)·곡선 목록·12곡 corpus·최신 head CI·리뷰까지 검증한다.
  병합/VM배포는 별도 승인이다.

### 시스템 경계 타이 5개 (D-073)

- m3 A4/C5, m6 E4, m9 C5/E5를 대상으로 각 반쪽 후보의 생성·삭제·머리 연결·시스템 간 짝짓기를 단계 로그로 확인한다.
- 최종 그래프에서 보이지 않는 것을 모두 미검출로 취급하지 않는다. 2026-09-20 진단에서 m4 두 호와 m9 C5는
  생성 후 `Page.connectOrphanSlurs`가 미짝 후보로 삭제했다. m3 A4·m7 E4는 D-072/D-063의 양끝 머리 조건에서 거부된다.
  m10 E5는 원형 모델의 확장·최소 폭 조건으로 작은 호가 완성되지 않는다.
- 기준선 실패 fixture를 먼저 확보한다. 같은 음·같은 보표·같은 휨 방향의 경계 타이 후보를 일반 이음줄보다 먼저 짝짓고,
  제한적으로 복원한 반쪽은 상대 반쪽과 머리 근거가 모두 있을 때만 수용하는 후보를 검증한다.
- 새 복원 반쪽은 페이지 내부 경계와 기존 orphan 분류를 모두 만족해야 한다. 첫/마지막 시스템 바깥으로
  나가는 새 반쪽을 일반 slur로 남기지 않는다. 새 복원의 페이지 간 확장은 제외한다.
- normal/recovery 전체 빌드·native 정상/부정·Clair3회(이벤트191/191·타이42/43·오검출0 목표)·12곡 corpus·성능·
  타입/lint/Jest·자체 리뷰·최신 head CI/실제 리뷰까지 수행한다. m12 교차·D-065 후속·병합·운영 VM 접근 및 배포는 제외한다.
