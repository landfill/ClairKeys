...
3. **내비게이션 구성** — 목표는 `내 악보`·`새 악보`·`탐색` 3개다. 현재 Header는 로그인 시
   ...
   상태 표시로 흡수)은 DS0-2의 결론에 의존하며, **[DS-G1](DS-G1-processing-state-contract.md)의
   G1-4가 이를 확정한다. 따라서 DS-G1이 DS-1보다 먼저 끝난다.**
...
- 2026-08-29 — 사용자 검토를 반영해 DS 트랙의 의존성과 완료 기준을 보강했다: DS-7을 DS-2~DS-6 전부
  이후로, DS-6을 DS-5 이후로 옮기고, 처리 상태 출처를 확정하는 결정 gate **DS-G1**을 추가해 DS-3·DS-4의
  선행 조건으로 뒀다. 화면 소유 범위 표로 `/sheet/[id]` 충돌(DS-2 vs DS-5)을 해소하고, DS-G1과
  DS-1~DS-7의 phase 문서 8개와 이슈 #76 전체 완료 조건을 작성했다. DS-0 자체는 `DONE`을 유지한다.
