...
이 비용 모델 단계의 완료 조건은 충족했다. 이슈 #126의 나머지 문맥/표기 확장은 2026-09-20 사용자 현재 상태 마감 지시로
NOT_PLANNED 종료했다. 추가 문맥 전달이 구현됐다는 의미는 아니며, 현재 phrase-dp-v3의 성과를 유지한다.
...
- 음계와 순차 진행에서 같은 손가락이 3회 이상 연속되지 않는다. **넘김이 물리적으로 성립하지 않는 도약
  연속(음정이 `CROSSING_MAX_INTERVAL`을 넘는 구간)은 정당한 예외로 명시한다** — 넘김 후보가 아예 없어 모든
  후보가 같은 "손을 뗀다" 비용을 물므로, 한 손가락을 재사용하는 것이 실제 연주와 일치한다. 이슈 #126의 완료
  조건이 허용한 "정당한 예외는 근거와 함께 명시한다"에 해당한다. 회귀로 고정한다.
