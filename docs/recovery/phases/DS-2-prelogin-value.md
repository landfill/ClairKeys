...
## 2026-08-30 D-031 주 — 홈 샘플 체험은 DS-5가 걷어낸다

이 단계가 만든 홈의 자체 완결 샘플 체험은 **DS-5에서 제거된다.** 사용자가 홈 샘플을 정적
예시(GIF 등)로 바꾸기로 했고, 그 전 단계로 재생기를 걷어내고 영역만 확보한다 (D-031).

DS-2는 `DONE`을 유지한다. 이 단계는 당시 계획대로 만들었고, D-031은 그것을 되돌리는 것이 아니라
다음 단계에서 대체하는 것이다. 아래 완료 조건과 Progress 기록은 DS-2 완료 시점의 사실이다.

이 결정으로 바뀌는 판정: 완료 조건 1의 ①은 `실제 낙하 노트 결과`에서 `결과가 들어갈 영역의 확보`로
완화됐고 그 판정은 DS-5가 한다. 완료 조건 3(로그인 없이 실제 재생)의 판정자에서 DS-2가 빠지고
DS-6이 단독으로 판정한다. 이월 지적 C5(`tempoSource: 'user'`가 "직접 입력"으로 표시)는 재생기와
