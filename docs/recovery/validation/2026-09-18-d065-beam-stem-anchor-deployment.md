...
- 사용자가 배포 후 앱에서 Clair를 다시 변환하고 결과 애니메이션 JSON의 공개 URL을 제공했다(Supabase `animation-data` 버킷,
  `804629/omr_57da14f3-….json`, 회수본 sha256 `c13dfe00…`). 새 업로드·VM 접근·DB 변경은 없다.
- `generated_at` 2026-09-18T02:45:25Z = **11:45 KST**로 전환(11:27 KST) 이후 변환이다. tempo 69, 9/8, 157음, duration 65.000.
- **운영 157음이 로컬 검증 결과와 전 필드 완전히 같다**(midi·start·duration·hand·voice·staff 모두, 차이 0건).
  대조 대상은 워커 검증 실행 `issue134-beams-2026-09-17/codex-verification/clair/clair-1/animation.json`(빌드 `d065b-codex-verification`)이다.
  즉 Clair 171/191을 낸 실행과 운영 출력이 같다.
- **직전 운영(D-064, 2026-09-15 재변환본) 대비 변화는 m9 하나로 국한된다.** 8분음표 길이는 60/69/2 = 0.434783초다.
  - #0~#81(m1~m8, 31.52초 이전)은 완전히 동일하다. D-062·D-063·D-064의 수정이 모두 보존됐다.
  - #82 E3(왼손, 31.52174초): 8분음표 4개 → **2개**.
  - #83 G3(왼손): 8분음표 2개 → **1개**, 시작 32.39130 → 31.95652.
  - #84~#156(73음): 길이·음높이 그대로 시작만 **일제히 8분음표 1개(0.434783초)씩 앞당겨졌다**.
    당김량의 종류가 하나뿐이라 균일하다.
  - 음높이·손·staff는 **0건** 바뀌었다. 빔 수정이 음가만 바꾼다는 설계와 일치한다.
  - voice 번호만 m9 부근 8음에서 재부여됐다(v6→v1, v8→v6, v7→v5). midi·start·duration·hand·staff는 같다.
    이 재부여는 로컬 검증 결과와도 동일하므로 운영에서 새로 생긴 현상이 아니다.
  - 전체 duration 65.434783 → **65.000000**초로 정확히 8분음표 1개만큼 줄었다.
- 마디 위치 교차확인: 9/8·♩=69에서 한 마디는 8분음표 9개 = 3.913초이므로 m9는 31.30초에 시작한다.
  바뀐 첫 음의 시작이 31.52초이므로 변화 구간은 m9다.
- 한계: 이 비교는 canonical 애니메이션 기준이다. 원본 기준표 평가(171/191)는 MusicXML raw 이벤트로 하는데 서비스가 MusicXML을
  지우므로 운영에서 직접 재평가하지 않았다. 다만 canonical 결과가 171/191을 낸 로컬 실행과 전 필드 동일하다.
- 회수: `local-test-data/results/beam-deploy-2026-09-18/live-app-animation.json`(Git 제외).

## 한계와 남은 범위

- 운영 PDF 업로드 스모크나 웹 업로드→콜백→플레이어 E2E는 하지 않았다. 확인은 위 재변환 결과에 근거한다.
