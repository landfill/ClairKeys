...
## 사후 육안 확인 (2026-09-02, 병합 후)

사용자가 병합된 `main`의 재생 화면을 실측해 **통과를 보고했다.** 위 "검증하지 못한 것"의 첫 항목
(실제 브라우저·실기기 육안 확인)이 해소된다.

이 확인의 근거는 사용자의 관측이며 자동 검사가 아니다. 회귀 테스트는 좌표만 판정하므로, 흑건이 화면에서
어색하지 않다는 판정은 이 확인이 유일한 근거다. 나머지 두 미검증 항목(`PianoKeyboard.tsx` 경로 —
도달 불가, 낙하 노트와 건반의 정렬 육안 확인)은 그대로 남는다.
