...
지시했다. 신규 결함 9건을 DS-0 phase 문서의 **"발견된 결함 대장"**에 담당 단계와 함께 남겼다:
...
DS0-8 곡 제목 편집 고아 컴포넌트(DS-4), DS0-9 구간 반복 부재(DS-5).
**DS-1~DS-7 진입 시 이 대장에서 해당 단계 배정 항목을 먼저 읽는다.**

DS-1 진입 조건 6개도 확정했다 — 토큰 정의, 다크 모드 범위, 내비게이션 구성, 고아 라우트 처리,
