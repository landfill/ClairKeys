...
m9 빔 복원 [PR164](reviews/PR-164.md)(D-065, `bbcc09b`)는 2026-09-18 병합·배포됐다(image `9eef7512…`).
나머지 타이·리듬 오류는 남아 있다.
...
1. **D-065 운영 인식 결과를 사용자의 앱 재변환으로 확인한다(선택).** PR164는 2026-09-18 병합(`bbcc09b`)·배포됐다
   (image `9eef7512…`, 롤백 태그 `rollback-pr164-20260918`). 배포 승인에 PDF 스모크는 포함하지 않아 **운영 인식 결과는 미확인**이다.
   기대: Clair m9의 4분음표 두 개가 8분음표 두 개가 되고 원본 기준 이벤트가 160 → 171/191.
   사용자가 앱에서 Clair를 다시 변환하면 애니메이션 JSON을 로컬 검증 결과와 대조한다.
   [배포 근거](validation/2026-09-18-d065-beam-stem-anchor-deployment.md).
...
- **OMR 운영(2026-09-18)**: PR164 `bbcc09b` 배포 완료. image `9eef7512…`, 롤백 태그 `rollback-pr164-20260918`(`2a71ede5…`).
  ...
  **운영 인식 결과는 아직 확인하지 않았다**(PDF 스모크는 승인 범위 밖).
  [배포 근거](validation/2026-09-18-d065-beam-stem-anchor-deployment.md).
