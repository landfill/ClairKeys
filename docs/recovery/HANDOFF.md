...
**남은 것(DS3-2)**: 413이 "잠시 후 재시도"로 분류되는 경로는 그대로다. 한도를 4.5MB **정각**으로
맞춰 multipart 오버헤드 여유가 없으므로 근접 파일은 여전히 413이 된다.
...
| DS3-2 | **413 경로는 그대로다.** `uploadFailures.ts`에 `status === 413` 분기가 없어 `describeServiceUnavailable()`로 떨어지고 "잠시 후 재시도"를 권한다. 한도를 4.5MB에 **정각으로** 맞춰 multipart 오버헤드만큼의 여유가 없으므로, 4.5MB에 근접한 파일은 여전히 413이 되고 그때 성공할 수 없는 재시도를 안내받는다. 한도에 여유를 두거나(예: 4MB) 413 분기를 추가한다 | **미결, GitHub 이슈 미등록** |
