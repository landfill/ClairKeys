...
**DS-6 확인 대기**: PR #98은 `OPEN`, HEAD `340c595`, mergeState `CLEAN`이며 required `Lint`·`Security Audit`·`Run Tests`·양쪽 `E2E Tests`가 모두 초록이다. 사용자는 시크릿 창에서 탐색 → 상세 → 미리보기 → 로그인 → 같은 곡 복귀(이슈 #76 완료 조건 3의 실제 판정), 1440·1024·390 반응형, DS-5에서 넘어온 실기기 확인을 해야 한다. 카드 재생 시간 정규화와 `/sheet/[id]` 403/404 열거는 DS0-1과 함께 다룰 별도 후속 작업 후보이며 PR #98에서 고치지 않는다.
...
| **DS6-DURATION** | `SheetMusic`에 카드용 duration 값이 없고 애니메이션 JSON에만 있어 공개 카드에서 곡 길이를 비교할 수 없다 | **후속 데이터 모델 작업 후보** |
