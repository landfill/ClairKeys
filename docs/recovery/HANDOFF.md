...
1. **[PR168](reviews/PR-168.md) 사용자 승인으로 병합 완료** (`6de51f1`, 2026-09-19). 현재 head CI/실제 재리뷰를 재확인하고 병합했으며 로컬 main에도 반영했다.
   ...
   정확한 병합 커밋의 VM 배포를 진행 중이다. [배포 기록](validation/2026-09-19-d069-m1-dot-deployment.md).
2. **운영은 PR167 `983caf9` 그대로다.** m1 개선187/191은 로컬 결과이며 운영 재변환 결과가 아니다. 사용자가 VM 배포를 승인했으며 검증 후 전환한다.
