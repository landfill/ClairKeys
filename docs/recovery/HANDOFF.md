...
추가 수정·5브라우저 E2E 검증을 마쳤다. 새 preview·CI·재리뷰 확인 중.
...
- PR183 `12e5607` preview에서 92의 20→25초 초과 줄 스크롤이 상단을 유지하는지 재확인하고 최신 CI/리뷰를 처리한다. 이어 현재 `/sheet/[id]`의 미작동 모드 설정 제거를 별도 브랜치·PR로 진행한다. 어느 PR도 사용자 승인 전 병합하지 않는다.
