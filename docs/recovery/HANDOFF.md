...
1. **#134 m3 둘잇단4건을 로컬에서 조사·수정 중.** 작업 브랜치 `codex/issue-134-m3-duplet`,
   최신 main 시작점 `ff88b8d`, 기준선 PR168/d069b에서187/191·타이30/43을 새 실행으로 재확인했다.
2. 원본의 두 숫자2는 슬러와 접촉하고 곡선 glyph에 포함되어 SYMBOLS에서 거의 지워진다.
   엔진에는 둘잇단 shape·3/2 배율도 없다. native fixture는 양쪽 기준선에서 실패했다(`cdb43d9`).
   [단계별 근거·진행](validation/2026-09-19-issue-134-m3-duplet.md).
3. 다음: 일반화 가능한 숫자 판별과 엔진 내부 둘잇단 지원, 전체 Docker/native/Clair3회/corpus12곡 검증,
   자체 리뷰와 non-draft PR 및 최신 head CI·실제 리뷰 대응. 아직 구현 완료가 아니다.
4. 다른 원인의 누락 타이 전체와 D-065 후속은 별도다. main 병합·운영 배포·유료 서비스는 미승인이다.
