...
| 7 | P2-A | 중복 계층과 설정 정리 | IN_PROGRESS | `codex/p2-architecture-cleanup` | P1-A~B |
...
| 7 | DS-6 | 탐색과 공개 체험 (`/sheet/[id]` 로그인 전 접근 포함) | DONE | `codex/ds-6-explore` | DS-2, **DS-5** |
| 8 | DS-7 | 알림·빈 화면·오류 상태 완결 | DONE | `codex/ds-7-states` | **DS-2 ~ DS-6 전부** |
...
- 홈의 **자체 완결 샘플 체험** (`/sheet/[id]`를 쓰지 않는다) — **DS-5가 D-031로 걷어낸다**
...
- **홈에서 재생기를 걷어내고 낙하 노트 결과가 들어갈 영역만 확보** (D-031). 정적 예시를 채우는
  것은 후속 작업이다
- 재생·정지, 속도, **구간 반복**을 1차 컨트롤로 (DS0-9). 홈이 빠지면 `PlaybackControls`는 공유가
  아니게 되므로 D-019 결정 8이 더 이상 이를 막지 않는다 (D-031)
...
- 공개 악보를 곡명·작곡가·난이도 중심으로 정리 (DS0-4)
...
| # | 조건 | 판정 방법 | 최종 판정 단계 |
|---|---|---|---|
| 1 | 홈 최초 뷰포트(1440×900, 스크롤 0) 안에 ①낙하 노트 결과가 들어갈 영역 ②`PDF → 변환 → 시각적 연습` 3단계 ③주 CTA가 모두 보인다 (D-031) | 관측 가능 | DS-2(②③), DS-5(①) |
| 2 | 주요 CTA가 `내 악보로 시작하기`로 일관된다 | 문자열 검사 | DS-2 |
| 3 | 로그인하지 않은 상태에서 실제 학습 결과를 최소 한 번 재생할 수 있다 | 시크릿 창 수동 확인 | DS-6(공개 악보) — D-031로 홈 샘플이 빠져 DS-2는 판정자가 아니다 |
...
**2026-08-30 DS-7 최종 판정:** DS-7 PR #99가 merge commit `3eaee81`로 병합됐고, 이슈 #76
완료 조건 7개를 모두 `충족`으로 판정했다. 조건 3·4·7은 자동 검사가 아니라 사용자가 2026-08-30
수동 확인을 수행하고 통과를 보고한 것이 근거다. DS-0의 회귀 계약(재생 기하 상수 7개, D-019 가로
