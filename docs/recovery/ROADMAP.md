...
**2026-08-30 DS-6 판정:** DS-6은 PR #98 병합으로 코드 완료 상태가 됐지만, 이슈 #76 완료 조건
3의 실제 판정자인 DS-6이 시크릿 창에서 로그아웃 탐색 → 상세 → 미리보기 → 로그인 → 같은 곡 복귀를
아직 확인하지 않았다. E2E는 탐색·클릭 보조 근거일 뿐 실제 재생을 판정하지 않으므로 조건 3은 아직
`충족`으로 표시하지 않는다.
