"""Executable regression: a beam overshooting its stems still beams its eighths.
...
from curve_along_beam_fixture import EXPECTED_MEASURES, write_pdf
...
class CurveAlongBeamNativeTests(unittest.TestCase):
    def test_each_engine_keeps_the_beam_whose_ends_run_past_their_stems(self):
        ran = False
        for label, configured in ENGINES:
            ...
            with self.subTest(engine=label), tempfile.TemporaryDirectory() as tmp:
                ...
                for measure, expected in zip(measures, EXPECTED_MEASURES):
                    ...
                    self.assertEqual(
                        found,
                        [(expected["eighths"], "eighth", divisions // 2)] * 2
                        + [(expected["quarters"], "quarter", divisions)] * 2,
