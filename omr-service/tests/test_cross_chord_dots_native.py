"""Executable regression: a dot between two voices' line heads goes to the lower head.
...
from cross_chord_dots_fixture import EXPECTED_MEASURES, write_pdf
...
class CrossChordDotsNativeTests(unittest.TestCase):
    def test_each_engine_gives_the_dot_to_the_lower_voice_whichever_chord_comes_first(self):
        ran = False
        for label, configured in ENGINES:
            ...
            with self.subTest(engine=label), tempfile.TemporaryDirectory() as tmp:
                ...
                for measure, expected in zip(measures, EXPECTED_MEASURES):
                    ...
                    self.assertEqual(
                        found,
                        sorted([(expected["upper"], "quarter", 0, divisions)] * 3
                               + [(expected["lower"], "half", 1, 3 * divisions)]),
