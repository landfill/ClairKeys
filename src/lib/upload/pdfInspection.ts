...
/**
 * Vercel Function의 요청 본문 한도가 4.5MB다. 그보다 큰 본문은 라우트 코드가 실행되기도 전에
 * 플랫폼이 413으로 끊으므로, 화면이 통과시킨 파일이 서버에 닿지 못하는 구간이 생긴다 (D-032).
 *
 * 한도를 4.5MB에 정각으로 맞추지 않고 **4MB로 여유를 둔다** (D-033). 서버가 받는 것은 파일이
 * 아니라 multipart 본문이고, 여유가 없으면 두 가지가 경계를 넘긴다:
 *
 * 1. **MB 해석** — 플랫폼 문서의 `4.5MB`가 십진이면 4,500,000바이트다. 이진으로 읽은
 *    `4.5 * 1024 * 1024`는 4,718,592라 그것보다 218,592바이트 크다. 오버헤드보다 100배 큰
 *    차이이고, 어느 해석이 맞는지는 앱이 정할 수 없다.
 * 2. **multipart 오버헤드** — 폼은 파일 외에 `title`·`composer`·`tempo`·`categoryId`·
 *    `isPublic`을 함께 보낸다. 경계 문자열과 파트 헤더가 약 2KB를 더한다.
 *
 * 4 * 1024 * 1024 = 4,194,304는 두 해석 모두에서 안전하고, 십진 기준으로도 약 300KB가 남는다.
 *
